  ...
  <div class="header">
    <UContainer>
      ...
      <div class="lg:hidden flex">
        ...
        <!-- Menu -->
        <div v-if="isMenuOpen" class="mobile-menu absolute top-12 left-0 w-full bg-white shadow-lg z-10">
          <ul class="flex flex-col items-center p-4">
            <li v-for="(item, index) in items[0]" :key="index" class="py-2">
              <NuxtLink :to="item.to" class="flex items-center text-xl" @click="toggleMenu">
                <UIcon :name="item.icon" class="size-7 mr-2" />
                {{ item.label }}
              </NuxtLink>
            </li>
          </ul>
